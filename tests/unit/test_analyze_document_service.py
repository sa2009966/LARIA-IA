...
class TestAnalyzeDocumentService:
    ...
    async def test_answer_question_pasa_contexto_y_pregunta(
        ...
        published = event_bus_mock.publish.await_args.args[0]
        assert published.event_type == "TutorQuestionAskedEvent"
        assert published.question == "¿Qué es?"
        assert published.answer == "Respuesta corta."
        assert published.student_id == owner_id
        assert published.document_id == doc.id
