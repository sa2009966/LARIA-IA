...
class LearningEvidenceProjector:
    ...
    async def handle_tutor_question(self, event: TutorQuestionAskedEvent) -> None:
        ...
        if self._profile_repo is None:
            return
        if (event.signal_kind or "none") == LearningSignalKind.NONE.value:
            # Aun sin struggle: registrar estilo preferido si viene
            if event.cognitive_style and self._profile_repo is not None:
                ...
                async def _persist_style():
                    ...
                    if event.cognitive_style:
                        p.pedagogical_memory.set_preferred_style(event.cognitive_style)
                    if event.pedagogical_mode:
                        p.pedagogical_memory.remember_strategy(event.pedagogical_mode)
                    await self._profile_repo.save(p)
                    return p

                await with_concurrency_retry(_persist_style)
            return
